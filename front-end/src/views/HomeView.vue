<template>
  <main>
    <h1>Pizze</h1>
    <form v-if="!showCreateForm && !showUpdateForm" @submit.prevent="getAllPizzas">
    ...
    <form v-if="showUpdateForm" @submit.prevent="pizzaUpdate">
      <label for="name">Name</label>
      <br />
...
function clearForm() {
  showCreateForm.value = showUpdateForm.value = false

  newPizza.value = ref({ ...emptyPizza })
}
function storePizza() {
  axios
    .post(API_URL + '/pizze', newPizza.value)
    .then(() => {
      clearForm()
    })
...
function pizzaEdit(id) {
  for (const pizza of pizze.value) if (pizza.id == id) editPizza.value = { ...pizza }

  showUpdateForm.value = true
}
function pizzaUpdate() {
  axios
    .put(API_URL + '/pizze/' + editPizza.value.id, editPizza.value)
    .then(() => {
      clearForm()
      getAllPizzas()
    })
